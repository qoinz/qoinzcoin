...
*Setup instructions for a gitian build of qoinz using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the qoinz
...
Clone the git repositories for qoinz and gitian and then checkout the qoinz version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the qoinz repository
...
To build qoinz (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the qoinz repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/qoinz/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/qoinz-project/qoinz
...
For example:
```bash
URL=https://github.com/qoinz-project/qoinz.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit qoinz=${COMMIT} --url qoinz=${URL} ../qoinz/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit qoinz=${COMMIT} --url qoinz=${URL} ../qoinz/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit qoinz=${COMMIT} --url qoinz=${URL} ../qoinz/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/qoinz-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/qoinz-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/qoinz-build.assert
```
...
[qoinz/gitian.sigs](https://github.com/qoinz-project/gitian.sigs/) repository, or if that's not possible to create a pull
request.
