...
#if defined(NDEBUG)
#error "qoinz cannot be compiled without assertions."
#endif
...
unsigned int nStakeMinAge = 5 * 60 * 60; // 600 Block  // 5 hour
int64_t nReserveBalance = 0;

/** Fees smaller than this (in uqoinz) are considered zero fee (for relaying and mining)
...
bool CheckTransaction(const CTransaction& tx, CValidationState& state)
{
    ...
    // Size limits
    if (::GetSerializeSize(tx, SER_NETWORK, PROTOCOL_VERSION) > QOINZ_MAX_BLOCK_SIZE)
...
bool AcceptToMemoryPool(CTxMemPool& pool, CValidationState& state, const CTransaction& tx, bool fLimitFree, bool* pfMissingInputs, bool fRejectInsaneFee, bool ignoreFees)
{
...

    {
        ...
        if (nSigOps > QOINZ_MAX_TX_SIGOPS)
            return state.DoS(0,
                error("AcceptToMemoryPool : too many sigops %s, %d > %d",
                    hash.ToString(), nSigOps, QOINZ_MAX_TX_SIGOPS),
...
CAmount GetBlockValue(int nHeight)
{
    ...
    if (Params().NetworkID() == CBaseChainParams::TESTNET){
        if ( nHeight == 0 ) {
            nSubsidy = 840000 * COIN; // premine testnet
        } else if (nHeight < 50) {
            nSubsidy = 1 * COIN;
        ...
        if ( nHeight == 0 ) {
            nSubsidy = 420000 * COIN; //  2% premine
        } else if (nHeight < 50) {
            nSubsidy = 0.5 * COIN;
        } else if (nHeight >= 50 && nHeight < 100) {
            nSubsidy = 0.8 * COIN;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++) {
        ...
        if (nSigOps > QOINZ_MAX_BLOCK_SIGOPS)
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    // Size limits
    if (block.vtx.empty() || block.vtx.size() > QOINZ_MAX_BLOCK_SIZE || ::GetSerializeSize(block, SER_NETWORK, PROTOCOL_VERSION) > QOINZ_MAX_BLOCK_SIZE)
...
int nLoaded = 0;
    try {
        ...
        CBufferedFile blkdat(fileIn, 2 * QOINZ_MAX_BLOCK_SIZE, QOINZ_MAX_BLOCK_SIZE + 8, SER_DISK, CLIENT_VERSION);
        ...
        while (!blkdat.eof()) {
            ...
            try {
                ...
                // read size
                blkdat >> nSize;
                if (nSize < 80 || nSize > QOINZ_MAX_BLOCK_SIZE)
                    continue;
