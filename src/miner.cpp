...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake)
{
    ...
    // Limit to betweeen 1K and QOINZ_MAX_BLOCK_SIZE-1K for sanity:
    nBlockMaxSize = std::max((unsigned int)1000, std::min((unsigned int)(QOINZ_MAX_BLOCK_SIZE - 1000), nBlockMaxSize));
...

    {
        ...
        while (!vecPriority.empty()) {
            ...
            if (nBlockSigOps + nTxSigOps >= QOINZ_MAX_BLOCK_SIGOPS)
                continue;
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("qoinzMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("qoinzMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("qoinzMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("qoinz-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        //LogPrintf("Running qoinzMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(), ::GetSerializeSize(*pblock, SER_NETWORK, PROTOCOL_VERSION));
