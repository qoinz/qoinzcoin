...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::QOINZ)
    {
    }
...
//    ui->label_qoinz4->setVisible(showImmature || showWatchOnlyImmature);
...
//        model->getWatchBalance(), model->getWatchUnconfirmedBalance(), model->getWatchImmatureBalance());
        ...
        connect(ui->blabel_qoinz, SIGNAL(clicked()), this, SLOT(openMyAddresses()));

        emit model->makeBalance();
    }

    // update the display unit, to not use the default ("qoinz")
    updateDisplayUnit();
}
...
void OverviewPage::updateMasternodeInfo()
{
  ...
  if (masternodeSync.IsBlockchainSynced() && masternodeSync.IsSynced())
  {
   ...
   std::vector<CMasternode> vMasternodes = mnodeman.GetFullMasternodeMap();
    ...
    CAmount tNodesSumm = mn1*5000 + mn2*50000;
  ...
  if (CurrentBlock >= 0) {
      ui->label_lcolat->setText("5000 QOINZ");
      ui->label_mcolat->setText("50000 QOINZ");
  }
...
void OverviewPage::updateBlockChainInfo()
{
    if (masternodeSync.IsBlockchainSynced())
    {
        ...
        double BlockReward = GetBlockValue(CurrentBlock);
        double BlockRewardqoinz =  static_cast<double>(BlockReward/COIN);
        ...
        ui->label_hashrate->setText(tr("Hashrate:"));
        ui->label_hashrate_value->setText(QString::number(netHashRate));		

        ui->label_CurrentBlockReward_value->setText(QString::number(BlockRewardqoinz, 'f', 1).append(" | ") + QString::number(GetSporkValue(SPORK_11_DEVFUND_FEE)).append("%"));

        ui->label_Supply_value->setText(QString::number(chainActive.Tip()->nMoneySupply / COIN).append(" QOINZ"));
...
void OverviewPage::on_pushButton_Website_clicked() {
    QDesktopServices::openUrl(QUrl("https://qoinz.network", QUrl::TolerantMode));
}
void OverviewPage::on_pushButton_Discord_clicked() {
    QDesktopServices::openUrl(QUrl("https://discord.gg/WfxZAVg", QUrl::TolerantMode));
}
void OverviewPage::on_pushButton_Telegram_clicked() {
    QDesktopServices::openUrl(QUrl("https://t.me/qoinznetwork", QUrl::TolerantMode));
}
void OverviewPage::on_pushButton_Twitter_clicked() {
    QDesktopServices::openUrl(QUrl("https://twitter.com/qoinnexion", QUrl::TolerantMode));
}

void OverviewPage::on_pushButton_Facebook_clicked() {
    QDesktopServices::openUrl(QUrl("https://www.facebook.com/Qoinnexion-104364084344517/", QUrl::TolerantMode));
}

void OverviewPage::on_pushButton_Explorer_clicked() {
    QDesktopServices::openUrl(QUrl("http://explorer.qoinz.network/", QUrl::TolerantMode));
}
