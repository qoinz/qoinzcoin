...
QFontDatabase::addApplicationFont(":/fonts/qoinz_font");

QString windowTitle = tr("QOINZ") + " - ";
#ifdef ENABLE_WALLET
...
void BitcoinGUI::createActions(const NetworkStyle* networkStyle)
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a QOINZ address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and QOINZ: URIs)"));
...
*/
    ...
    aboutAction = new QAction(networkStyle->getAppIcon(), tr("&About QOINZ wallet"), this);
    aboutAction->setStatusTip(tr("Show information about QOINZ wallet"));
    ...
    optionsAction->setStatusTip(tr("Modify configuration options for QOINZ wallet"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your QOINZ addresses to prove you own them"));
    verifyMessageAction = new QAction(QIcon(GUIUtil::getThemeImage(":/icons/transaction_0")), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified QOINZ addresses"));
    ...
    openAction->setStatusTip(tr("Open a QOINZ: URI or payment request"));

    showHelpMessageAction = new QAction(QApplication::style()->standardIcon(QStyle::SP_MessageBoxInformation), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the QOINZ help message to get a list with possible QOINZ command-line options"));
...
void BitcoinGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
    ...
    QString toolTip = tr("QOINZ client") + " " + networkStyle->getTitleAddText();
...
void BitcoinGUI::setNumConnections(int count)
{
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to QOINZ network", "", count));
}
...
{
    QString strTitle = tr("QOINZ"); // default title
...
static bool GetAndCompareVersion(BitcoinGUI* gui,  QString &version, QString &downloadUrl) {
    ...
    bool fHaveNew = false;
    bool fRet = false;
    QString url("https://binaries.qoinz.network/downloads/qoinzwallet/index.json");
    QString body;
    ...
    if(fRet) fRet = GetQtJsonObject(object, "qoinzwallet", walletObj);
    if(fRet) {
        ...
        #endif
        if(baseUrl.right(1) == "/"){
            downloadUrl = baseUrl + loaderStr + "?filename=qoinz-" + version + platformStr + suffixStr;
        }
        else{
            downloadUrl = baseUrl +  "/" + loaderStr + "?filename=qoinz-" + version + platformStr + suffixStr;
        }
...
void BitcoinGUI::checkVersion()
{
    ...
    message = QString(
                        ...
                        "max-width: 384px;"
                        "padding-left: 10px;\'>"
                        "A new version of Qoinz Desktop Wallet is available.<br/>"
